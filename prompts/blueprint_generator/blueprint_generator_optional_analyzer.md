...
**Step 2 — Optional Mathlib module analysis (`analyze_mathlib_module`)**:
Use `analyze_mathlib_module` only when it is likely to materially improve the blueprint.
It is especially useful when the relevant module is substantial, unfamiliar, or contains
a central theorem whose surrounding declarations may reveal the proof structure. It is
not required for every theorem or node, and you may skip it when semantic/name search and
the REPL provide enough information.

When using it, analyze the most important module first and analyze each important module
at most once. If the module name is uncertain, use `search_mathlib_docs` first — never
invent a module path. Treat the result as guidance, not ground truth: verify concrete
declaration names and signatures with `search_mathlib_docs` and `lean_run_code` before
committing them to the blueprint. Prefer `fetch_mathlib_source` only when a single
declaration needs direct inspection.
