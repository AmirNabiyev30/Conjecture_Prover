...
- **`analyze_mathlib_module`** *(optional)* — use it only when it is likely to materially improve the refactor. It is especially useful when the relevant module is substantial, unfamiliar, or contains a central theorem whose surrounding declarations may reveal the proof structure of a failing node. It is not required for every node, and you may skip it when semantic/name search and the prover diagnosis provide enough information. When using it, analyze the most important module first and analyze each important module at most once. It reads the module from the local `mathlib4` checkout and returns a structured summary (declarations, dependencies, proof strategy, Mathlib alignment, and possible decomposition). If it returns `Could not find module`, retry with a valid path from the name-lookup results — do not treat that error as analysis.
...
2. **Optional Mathlib module analysis** — use `analyze_mathlib_module` only when it is likely to materially improve the refactor (see Tools). When you do use it, treat the analysis as guidance, not ground truth: verify concrete declaration names with `search_mathlib_docs` before committing them. When you don't need it, skip this step and rely on semantic/name search.
